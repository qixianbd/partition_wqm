...
RCS_BASE(
    "$Id: symbols.cc,v 1.1.1.1 1998/06/16 15:15:10 brm Exp $")
...
sym_node::cvt_from_annote (annote *an, base_symtab * /* symtab */)
{
    ...
    char *new_name = an->immeds()->pop().string();
    if (parent() != NULL)
...
var_sym::var_sym (type_node *t, char *n)
{
    set_name(n);
    typ = t;
    defd = FALSE;
...
proc_sym::proc_sym (func_type *t, src_lang_type src, char *n)
{
    set_name(n);
    fse = NULL;
    file_pos = -1;
