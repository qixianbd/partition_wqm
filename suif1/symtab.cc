...
RCS_BASE(
    "$Id: symtab.cc,v 1.2 1999/08/25 03:29:17 brm Exp $")
...
base_symtab::base_symtab (char *n)
{
...
{
    char *n = name ? lexicon->enter(name)->sp : NULL;
...
sym_node *
base_symtab::lookup_sym (char *name, sym_kinds k, boolean up)
{
    assert(name != NULL);
...
var_sym *
base_symtab::new_var (type_node *t, char *n)
{
...
var_sym *
base_symtab::new_unique_var (type_node *t, char *base)
{
    char bfr[1024];
...
void
base_symtab::register_name_change (sym_node *the_sym, char *new_name)
{
    char *old_name = the_sym->name();
...
void
base_symtab::add_name_entry(sym_node *the_sym, char *new_name)
{
    if (new_name == NULL)
        return;
...
void
base_symtab::remove_name_entry(sym_node *the_sym, char *old_name)
{
    if (old_name == NULL)
        return;
...
static char *unique_type_name(char *base, base_symtab *syms);
...
char *
unique_type_name (char *base, base_symtab *syms)
{
    char bfr[1024];
    char *type_name;
    unsigned counter = 1;
...
global_symtab::global_symtab (char *n)
    : base_symtab(n)
{
...
proc_sym *
global_symtab::new_proc (func_type *r, src_lang_type src, char *n)
{ 
...
file_symtab::file_symtab(char *n, file_set_entry *fse)
    : global_symtab(n)
{
    fset = fse;
...
block_symtab::block_symtab (char *n)
    : base_symtab(n)
{
...
block_symtab *
block_symtab::new_unique_child (char *base)
{
    char bfr[1024];
    unsigned counter = 0;
...
label_sym *
block_symtab::new_unique_label (char *base)
{
    char bfr[1024];
...
proc_symtab::proc_symtab (char *n)
    : block_symtab(n)
{
    next_inum = 1;
