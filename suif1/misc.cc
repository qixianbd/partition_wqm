...
RCS_BASE(
    "$Id: misc.cc,v 1.2 1999/08/25 03:29:12 brm Exp $")
...
static char **warnings_seen = NULL;
...
char *__suif_assert_file = NULL;
int __suif_assert_line = -1;
char *__suif_assert_module = NULL;
...
void
__suif_assert (char *expr, char *file, int line, char *module)
{
    fflush(stdout);
...
static void find_file_and_line(suif_object *the_object, char **filename,
                               unsigned *linenum);
static void read_line_annote(immed_list *line_immeds, char **filename,
                             unsigned *line_num);
...
static void
find_file_and_line (suif_object *the_object, char **filename,
                    unsigned *linenum)
{
    if (filename != NULL)
        *filename = NULL;
...
static void
read_line_annote(immed_list *line_immeds, char **filename, unsigned *line_num)
{
...
void
error_line (int return_code, suif_object *the_object, char *fmt, ...)
{
    va_list ap;
    va_start(ap, fmt);
...
void
verror_line (int return_code, suif_object *the_object, char *fmt, va_list ap)
{
    unsigned src_line;
    char *src_file_name;
...
void
warning_line (suif_object *the_object, char *fmt, ...)
{
    va_list ap;
    va_start(ap, fmt);
...
void
vwarning_line (suif_object *the_object, char *fmt, va_list ap)
{
    if (ignore_warnings)
        return;
    ...
    if (warn_only_once)
      {
        if (warnings_seen == NULL)
          {
            warnings_seen = new char *[20];
            warning_record_length = 20;
            warnings_seen[0] = NULL;
          }
        else
          {
            char **follow_warnings = &(warnings_seen[0]);
            ...
            if (follow_warnings == &(warnings_seen[warning_record_length - 1]))
              {
                char **new_block = new char *[warning_record_length * 2];
                memcpy(new_block, warnings_seen,
                       warning_record_length * sizeof(char *));
...
announce_proc (proc_sym *the_proc)
{
    ...
    char *src_file_name = source_file_name(the_proc);
    if (src_file_name != NULL)
        fprintf(stderr, "%s: ", src_file_name);
                ...
                case FILE_OBJ:
                  {
                    ...
                    char *in_name = the_fse->in_name();
                    char *out_name = the_fse->out_name();
                    fprintf(stderr, "file");
                    if (in_name || out_name)
                        fprintf(stderr, " ");
                    if (in_name)
