...
RCS_BASE(
    "$Id: suifstreams.cc,v 1.2 1999/08/25 03:29:16 brm Exp $")
...
static int compare_ver_strings(char *ver_1, char *ver_2);
...
static void add_leuds(char *target, char *to_add);
...
base_stream::base_stream (char *n)
    : fp(0), pos(0), nm(n)
{
    flush_cache();
}
...
long
base_stream::hash (char *x)
{
    return (((unsigned long)x) >> 4) % CACHE_SZ;
}
...
void
out_stream::write_chunk (void *cp, size_t sz)
{
    check_open();
...
void
out_stream::write_string (char *s)
{
    int len = strlen(s) + 1;
    write_int(len);
    write_chunk(s, len);
...
void
out_stream::write_cached (char *s)
{
    int ndx = hash(s);
    if (cache[ndx] == s) {
	write_int(ndx);
...
in_stream::open ()
{
	...
	char *file_series_start_ver = read_string();
	char *file_system_ver = read_string();
...
in_stream::read_cached ()
{
    ...
    char *s1 = read_string();
    char *s2 = lexicon->enter(s1, TRUE)->sp;
    if (s2 != s1) delete[] s1;
