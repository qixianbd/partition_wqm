...
RCS_BASE(
    "$Id: tree_string_index.cc,v 1.2 1999/08/25 03:29:17 brm Exp $")
...
public:
    tsi_substring(unsigned long char_count, char *init_chars,
                  tsi_node *init_child);
    ~tsi_substring(void);
...
tsi_substring::tsi_substring(unsigned long char_count, char *init_chars,
                             tsi_node *init_child)
...
tsi_node *tree_string_index::follow_prefix_match(char *string,
...
void tree_string_index::internal_development_dump(char **buffer,
        ...
        boolean show_internals)
  {
    ...
    if (base_entry != NULL)
      {
        (*buffer)[position] = 0;
        fprintf(fp, "    (%p) \"%s\":", base_entry, *buffer);
        if (node_func != NULL)
...
si_entry *tree_string_index::enter(char *the_string, void *the_data)
  {
    ...
    switch (the_node->kind())
      {
        case TSI_SUBSTRING:
          {
            ...
            char *suffix = &(the_string[prefix_length + match_length + 1]);
            if (suffix[0] != 0)
              {
                tsi_substring *new_substring =
            ...
            if (match_length > 0)
              {
                char *prefix = &(the_string[prefix_length]);
        ...
        case TSI_CHAR_LIST:
          {
            ...
            unsigned long count = the_tsi_char_list->count();
            tsi_node *new_child = new tsi_leaf(result, this);
            char *suffix = &(the_string[prefix_length + 1]);
            if (*suffix != 0)
              {
                new_child =
                        new tsi_substring(strlen(suffix), suffix, new_child);
        ...
        case TSI_LEAF:
          {
            char *suffix = &(the_string[prefix_length]);
            tsi_leaf *new_leaf = new tsi_leaf(result, this);
...
si_entry *tree_string_index::lookup_entry(char *the_string)
  {
    assert(the_string != NULL);
    unsigned long prefix_length;
    tsi_node *the_node = follow_prefix_match(the_string, &prefix_length);
...
boolean tree_string_index::exists(char *the_string)
  {
    return (lookup_entry(the_string) != NULL);
  }

void *tree_string_index::lookup(char *the_string)
  {
